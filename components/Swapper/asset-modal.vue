...
<template>
  <div
    class="relative h-full w-full overflow-y-scroll bg-white p-4 scrollbar-thin scrollbar-track-transparent scrollbar-thumb-zinc-300 md:p-6 dark:border-zinc-700 dark:bg-zinc-900"
  >
    <h3 class="mb-4 w-full text-xl font-bold">
      Select a Network
      <button
        class="float-right -mt-2 rounded-full border border-zinc-300 p-1 text-center hover:bg-zinc-100"
        type="button"
    ...
    <div ref="focusGrid" class="grid grid-cols-2 gap-2">
      <button
        ...
        :key="chain.name"
        type="button"
        class="duration-400 rounded-md border p-1 text-left transition-all hover:bg-zinc-100 focus:outline dark:bg-zinc-700"
        :class="{
